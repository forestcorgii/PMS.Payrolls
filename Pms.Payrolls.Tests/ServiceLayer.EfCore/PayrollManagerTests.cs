...
namespace Pms.Payrolls.ServiceLayer.EfCore.Tests
{
    public class PayrollManagerTests
    {
        ...
        {
            Payroll expected = GenerateSeedPayroll("DYYJ", "2207-1", BankType.LBP, 19000, 20000, 17000, -1000, -1000, -1000);

            _payrollManager.SavePayroll(expected);

            Payroll actual = _payrollProvider.GetPayrolls("2207-1", BankType.LBP).Where(p => p.PayrollId == expected.PayrollId).FirstOrDefault();
