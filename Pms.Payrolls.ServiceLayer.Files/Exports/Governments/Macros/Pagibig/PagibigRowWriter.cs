...
namespace Pms.Payrolls.ServiceLayer.Files.Exports.Governments.Macros
{
    public class PagibigRowWriter : IRowWriter
    {
        public void Write(IRow row, Payroll payroll, int sequence)
        {
            ...
            row.GetOrCreateCell(4).SetCellValue(payroll.EmployeePagibig);// should be employer
            row.GetOrCreateCell(5).SetCellValue(payroll.EmployeePagibig + payroll.EmployeePagibig);// should be employer
        }
